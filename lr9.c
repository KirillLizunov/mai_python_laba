...
int count_i(int i, int j, int l, int k) {
    int new_i = ((((i * i) / (abs(j - l) + k + i)) - ((j * j) / (abs(i - l) + k + i))) % 30);

    return new_i;
}

int count_j(int i, int j, int l, int k) {
    int new_j = (sign(l) * min(i, j) - sign(j) * max(i, l) + k);

    return new_j;
}

int count_l(int i, int j, int l, int k) {
    int new_l = (i - j) * (j - l) * (l - i) % 20;
...
int main() {
    ...
    for (int i = 1; i <= 50; ++i){
        ...
        if (check(current_point)) {
            printf("\nОтановилось на итерации: %d\nФинальные значения i: %d, j: %d, k: %d", current_point.iteration, current_point.i, current_point.j, current_point.l);
            return 0;
        }
    ...
    printf("Не получилось, не фортануло...");
    return 0;
}
